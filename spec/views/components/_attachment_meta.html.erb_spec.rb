RSpec.describe "components/_attachment_meta.html.erb" do
  it "can include attribute metadata" do
    allow(view).to receive_messages(attachment: { content_type: "application/pdf",
                                                  file_size: 2048,
                                                  number_of_pages: 2 })
    render
    expect(rendered)
  ...
  it "can show file type that doesn't have an abbreviation" do
    allow(view).to receive_messages(attachment: { content_type: "text/plain" })
    render
    expect(rendered).to match(/Plain Text/)
  end

  it "doesn't show metadata information when there isn't any to show" do
    allow(view).to receive_messages(attachment: { content_type: "unknown/type" })
    render
  ...
  it "shows reference details on the first metadata line if provided" do
    allow(view).to receive_messages(attachment: { filename: "register.csv",
