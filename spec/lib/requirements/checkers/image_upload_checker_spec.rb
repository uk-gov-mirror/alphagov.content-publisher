...
RSpec.describe Requirements::Checkers::ImageUploadChecker do
  describe "#errors" do
    it "returns no issues if there are none" do
      file = fixture_file_upload("files/960x640.jpg", "image/jpeg")
      issues = Requirements::Checkers::ImageUploadChecker.new(file).issues
      expect(issues).to be_empty
    end

    it "returns an issue when no image is specified" do
      issues = Requirements::Checkers::ImageUploadChecker.new(nil).issues
