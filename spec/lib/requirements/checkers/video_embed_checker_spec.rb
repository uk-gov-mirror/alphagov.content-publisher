...
RSpec.describe Requirements::Checkers::VideoEmbedChecker do
  describe "#pre_embed_issues" do
    it "returns no issues when there are none" do
      issues = Requirements::Checkers::VideoEmbedChecker.new.pre_embed_issues(
    ...
    it "returns an issue for invalid URL hosts" do
      issues = Requirements::Checkers::VideoEmbedChecker.new.pre_embed_issues(url: "/on-localhost")
    ...
    it "returns an issue for an invalid URI" do
      issues = Requirements::Checkers::VideoEmbedChecker.new.pre_embed_issues(url: "http: youtube com")
      expect(issues).to have_issue(:video_embed_url, :non_youtube)
    end

    it "returns an issue for non-YouTube URLs" do
      issues = Requirements::Checkers::VideoEmbedChecker.new.pre_embed_issues(url: "http://vimeo.com")
      expect(issues).to have_issue(:video_embed_url, :non_youtube)
    end

    it "returns an issue for incomplete YouTube URLs" do
      issues = Requirements::Checkers::VideoEmbedChecker.new.pre_embed_issues(url: "https://www.youtube.com/watch")
      expect(issues).to have_issue(:video_embed_url, :non_youtube)
    end
  end
end
