...
RSpec.describe Requirements::Checkers::WithdrawalChecker do
  describe "#pre_withdrawal_issues" do
    let(:edition) { build(:edition) }

    it "returns no issues if there are none" do
      explanation = SecureRandom.alphanumeric
      issues = Requirements::Checkers::WithdrawalChecker.new(explanation, edition).pre_withdrawal_issues
      expect(issues).to be_empty
    end

    it "returns an issue if there is no public explanation" do
      issues = Requirements::Checkers::WithdrawalChecker.new(nil, edition).pre_withdrawal_issues
