...
RSpec.describe Requirements::Checkers::FileAttachmentChecker do
  describe "#pre_upload_issues" do
    it "returns no issues if there are none" do
      file = fixture_file_upload("files/text-file-74bytes.txt", "text/plain")
      issues = Requirements::Checkers::FileAttachmentChecker.new(file: file, title: "Cool title").pre_upload_issues
    ...
    it "returns no upload issues for a text file when it has no extension" do
      ...
      issues = Requirements::Checkers::FileAttachmentChecker.new(file: file, title: nil).pre_upload_issues
    ...
    it "returns an issue when there is no title" do
      issues = Requirements::Checkers::FileAttachmentChecker.new(file: nil, title: "").pre_upload_issues
      expect(issues).to have_issue(:file_attachment_title, :blank)
    end

    it "returns an issue when the title is too long" do
      max_length = Requirements::Checkers::FileAttachmentChecker::TITLE_MAX_LENGTH
      title = "z" * (max_length + 1)
      issues = Requirements::Checkers::FileAttachmentChecker.new(file: nil, title: title).pre_upload_issues
    ...
    it "returns an issue when no file_attachment is specified" do
      issues = Requirements::Checkers::FileAttachmentChecker.new(file: nil, title: "Cool title").pre_upload_issues
  ...
  describe "#pre_update_issues" do
    it "returns no issues if there are none" do
      file = fixture_file_upload("files/text-file-74bytes.txt", "text/plain")
      issues = Requirements::Checkers::FileAttachmentChecker.new(file: file, title: "Cool title").pre_update_issues
      expect(issues).to be_empty
    end
    ...
    it "returns title issues when only the title is provided" do
      max_length = Requirements::Checkers::FileAttachmentChecker::TITLE_MAX_LENGTH
      title = "z" * (max_length + 1)
      issues = Requirements::Checkers::FileAttachmentChecker.new(title: title).pre_update_issues
      expect(issues).to have_issue(:file_attachment_title, :too_long, max_length: max_length)
      expect(issues.items_for(:file_attachment_upload)).to be_empty
    end
  end
