...
RSpec.describe "Documents" do
  ...
  describe "PATCH /documents/:document" do
    ...
    context "when there are requirement issues" do
      let(:edition) { create(:edition, title: "Valid title") }

      it "returns an unprocessable response with an error message" do
        patch document_path(edition.document),
      ...
      it "maintains the submission" do
        patch document_path(edition.document),
              params: { revision: { title: "" } }
        expect(response.body).to have_tag('[name="revision[title]"]', seen: "")
      end
    end
  end
