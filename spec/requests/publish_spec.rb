...
RSpec.describe "Publish" do
  ...
  describe "GET /documents/:document/publish" do
    ...
    it "redirects to document summary with a flash when edition isn't publishable" do
      edition = create(:edition, summary: "")
      get publish_path(edition.document)

      expect(response).to redirect_to(document_path(edition.document))
      follow_redirect!
      expect(response.body).to have_tag(".gem-c-error-summary") do
        with_text(I18n.t!("requirements.summary.blank.summary_message"))
      end
    end
