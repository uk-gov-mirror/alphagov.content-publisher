module WhitehallImporter
  class CreateEdition
    attr_reader :document_import, :current, :whitehall_edition, :edition_number, :user_ids, :document_change_history

    def self.call(*args)
      new(*args).call
    end

    def initialize(document_import:, whitehall_edition:, current: true,
                   edition_number: 1, user_ids: {}, document_change_history: [])
  ...
  private
    ...
    def change_history(number: edition_number)
      change_history = document_change_history.select do |edition_change_history|
        edition_change_history["edition_number"] < number
      end

      change_history.map { |history| history["change_history_entry"] }
    end
