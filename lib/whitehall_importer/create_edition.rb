...
module WhitehallImporter
  class CreateEdition
    attr_reader :context, :current, :whitehall_edition, :edition_number, :user_ids

    def self.call(*args)
      new(*args).call
    end

    def initialize(context:, whitehall_edition:, current: true, edition_number: 1, user_ids: {})
      @context = context
      @current = current
  ...
  private
    ...
    def revision
      @revision ||= CreateRevision.call(context, whitehall_edition)
    end
